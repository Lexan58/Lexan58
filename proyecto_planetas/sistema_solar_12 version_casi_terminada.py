...
class CuerpoCeleste(Entity):  # Volver a usar Entity normal
    ...
    def create_orbit(self):
        ...
        self.orbit = Entity(
            ...
            color=color.white,
            position=(0, 0, 0),
            parent=scene,  # 👈 asegúrate de que esté en el árbol de escena
            enabled=True,
            name=f"orbit_{self.name}"
    )

    def update_position(self, time_value, orbita_activa):
    # Siempre actualizar la posición del planeta
...
class SistemaSolar:
    ...
    def setup_escena(self):
        ...
        self.info = Text(text='', origin=(0, 4), background=True, scale=1, max_lines=6, position=(0.4, 0.4)) #Circulo negro 
        self.info.background.color = color.rgba(255, 255, 255, 0)  # Fondo invisible
...
for e in scene.entities:
    if (
        ...
        and (not hasattr(e, 'texture') or e.texture is None)
        and not isinstance(e, Text)
        and not isinstance(e, Button)
        and not isinstance(e, Audio)
        and not isinstance(e.model, Mesh)
        and 'light' not in e.name.lower()
    ):
