...
# ==============================
# 🔒 Desactiva la interfaz de desarrollo (evita círculos como cog_menu_info)
from ursina import window
window.make_editor_gui = lambda *a, **k: None

app = Ursina(title="Sistema Solar 3D", borderless=False, fullscreen=False,
             vsync=True, development_mode=False)  # <‑‑ development_mode desactivado

window.color = color.black
# window.exit_button.visible = True  # Ya no existe en versiones recientes de Ursina
#window.fps_counter.enabled = True

# ------------------------------------------------------------------
# 🧹 Limpieza específica de círculo negro sospechoso (opcional)
# ------------------------------------------------------------------
def eliminar_circulo_negro():
    eliminados = 0
    for e in scene.entities:
        if (e.model == 'circle' and
            e.texture is None and
            e.color.r < 0.05 and e.color.g < 0.05 and e.color.b < 0.05 and
            e.color.a > 0.8 and
            e.position == Vec3(0, 0, 0)):
            print(f"🗑️ Eliminando posible círculo negro: {e}")
            destroy(e)
            eliminados += 1
    print(f"✅ Eliminados {eliminados} círculos sospechosos.")

invoke(eliminar_circulo_negro, delay=0)
invoke(eliminar_circulo_negro, delay=1)
...
# Resto del código original (planetas, órbitas, UI, etc.) ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
# ----------------------------------------------------------------------

# ...  (por brevedad se omite, pero TODO el bloque restante del script
#       original debe mantenerse sin cambios)
...
for e in scene.entities:
    if (
        e.parent == camera.ui or
        e.name.startswith('cog_menu') or
        (hasattr(e, 'model') and e.model == 'circle' and e.texture is None and e.position == Vec3(0) and e.color.a > 0.8)
    ):
        print(f"🧹 Eliminando overlay: {e}")
        destroy(e)
