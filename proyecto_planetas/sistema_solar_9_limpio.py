...
def debug_entity_init(self, *args, **kwargs):
    ...
    # Detectar entidades problemáticas más específicamente
    if (hasattr(self, 'position') and self.position == Vec3(0, 0, 0) and
        hasattr(self, 'model') and self.model in ('sphere', 'circle') and
        hasattr(self, 'texture') and self.texture is None and
        hasattr(self, 'color') and (self.color == color.black or self.color == color.clear)):
        
        print('\n🚨 ENTIDAD SOSPECHOSA DETECTADA:')
        ...
        # DESTRUIR INMEDIATAMENTE si es problemática
        if self.model == 'circle' and self.texture is None:
            print('🔥 DESTRUYENDO CÍRCULO PROBLEMÁTICO')
            try:
                destroy(self)
            except:
                pass
...
class SistemaSolar:
    ...
    def setup_escena(self):
        ...
        # USAR CUBOS EN LUGAR DE CÍRCULOS para evitar el problema
        self.planeta_iconos = []
        for i, planeta in enumerate(self.planetas):
            icono = Entity(
                model='cube',  # CAMBIO CRÍTICO: usar cubo en lugar de círculo
...
# LIMPIEZA INICIAL MÁS AGRESIVA
print("🧹 Iniciando limpieza de círculos negros...")
invoke(sistema_solar.limpiar_circulos_negros, delay=0.1)  # Limpiar después de que todo se cargue


def eliminar_esferas_negras():
    eliminados = 0
    for e in scene.entities.copy():
        try:
            if (e.model == 'sphere' and 
                (not hasattr(e, 'texture') or e.texture is None) and 
                e.position == Vec3(0,0,0) and 
                (not hasattr(e, 'name') or e.name != 'SOL_PRINCIPAL')):
                print(f"🧹 Eliminando esfera negra sospechosa: {e}")
                destroy(e)
                eliminados += 1
        except:
            pass
    print(f"✅ Esferas negras eliminadas: {eliminados}")

invoke(eliminar_esferas_negras, delay=0.2)
