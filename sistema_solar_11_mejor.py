...
class SistemaSolar:
    ...
    def setup_escena(self):
        ...
        self.info = Text(text='', origin=(0, 4), background=True, scale=1.5, max_lines=6)
    ...
    def seleccionar_planeta(self, index):
        try:
            ...
            if hasattr(self, 'info') and isinstance(self.info, Text):
                ...
                descripcion = self.info_expandida[index]
                distancia = self.distancias[index]
                tamano = self.tamanos[index]

                info_texto = f"{nombre}\n{descripcion}\nDistancia: {distancia} AU\nTamaño: {tamano}"
                if len(info_texto) > 500:
                    info_texto = info_texto[:497] + '...'

                self.info.text = info_texto
            else:
