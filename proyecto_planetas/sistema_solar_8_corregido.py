...
class DebugEntity(OriginalEntity):
    def __init__(self, *args, **kwargs):
        ...
        # Lista blanca de entidades permitidas sin textura
        allowed_entities = [
            'background', 'sol', 
            'orbita_', 'debug_entity',
            'ursina_mesh'  # Para las órbitas
        ]
        
        if not hasattr(self, 'texture') and \
           self.position == Vec3(0,0,0) and \
           not any(name in str(self.name) for name in allowed_entities):
            print(f"\n[ELIMINADO] Entidad sospechosa: {self}")
            destroy(self)
...
def limpieza_inicial():
    # Eliminar entidades no deseadas
    for e in scene.entities:
        if isinstance(e, Entity) and not hasattr(e, 'texture'):
            if e.model in ('sphere', 'quad') or e.position == (0,0,0):
                if not hasattr(e, 'name') or e.name not in ('sol', 'background'):
                    destroy(e)
    
    # Limpieza de luces
    if hasattr(scene, 'lights'):
        for light in scene.lights:
            if not hasattr(light, '_ursina_light'):
                light.disable()
...
print(f"Textura del sol: {sol.texture}")  # Debe mostrar la ruta correcta
...
for i, nombre in enumerate(nombres):
    btn = Button(text=nombre, position=Vec2(-0.8892508, 0.45) + Vec2(0.11, -0.07*i), scale=(0.2, 0.05), color=color.azure, on_click=Func(seleccionar_planeta, i))
    botones.append(btn)
